...
- **Reused in-repo:** `Matrix_Construction_CRMvsVoice.md` (Jobber Tier-2 + bundled AI Receptionist; CRM 53 / Voice 60; the attribution reframe), `SaaS_Gap_Research_Consolidated_Report.md` (clean-data = #1 corroborated moat; attribution = founder's unique asset; data-led wedge), `Physio_Product_Concepts.md` (PatientROI 76 — the cross-account-attribution template). Modeled COGS/margin figures tagged `[E]`.
