...
- **★ PHYSIO CONCEPT WINNER — "PatientROI" = 76/100 (lead candidate so far).** Cross-clinic acquisition intelligence ("Triple Whale for physio"): connects ad spend → lead → *booked paying patient* and benchmarks each clinic's cost-per-booked-patient vs an anonymized cross-clinic dataset. **The ONLY concept where Jaydeep's data IS the product** (data-fit 10/16, moat 9/12 — the boxes every other physio idea fails). Jane abstains from ad/marketing data → no kill-zone. 85%+ margin, $149–399/mo flat (no per-minute bleed). Upsell: **ReactivatePro** (lapsed-patient win-back) 71/100. Dropped: voice 54, reviews 47. **Jane App = Tier 3 partner-gated; but Juvonno (open API) + Cliniko (public REST) are Tier 1–2 → build on the open SoRs, use Jane read-only. WORKAROUND (Jaydeep's insight): Jane → Google Calendar sync → read booking events via Google Calendar API (Tier 1–2), bypassing Jane's gated API → the booking signal without partner approval. Verify GCal feed detail + PHIPA on patient data through Google.** Key stat: 74% of clinic owners can't tell cost-per-new-patient by channel; only 25.8% know their CAC. **Kill-criterion (locked): "yes" = an EXTERNAL clinic pays ≥$149/mo for the benchmark — owned-clinic enthusiasm doesn't count.** Evidence: `physio_concepts.md`.
