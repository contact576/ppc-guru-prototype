...
**★ Evidence-grounded refinement:** the validated job ranking, capacity-segmented inbound scripts, outbound win-back script, and quote-justified tone/guardrails now live in **`MODULE_A_script.md`** (grounded in `vertical/VOICE_OF_CUSTOMER.md` — ~150 verbatim clinic-owner/front-desk quotes). This file remains the architecture/COGS/compliance/locked-rules spec; the script file is the build target for conversation design. **Ship-first decision (from the evidence): inbound after-hours/overflow capture + instant missed-call text-back** — highest pain, lowest risk, the one job no one can call "salesy" (the patient initiated it).
